...
func matchScore(a Token, b Token) int {
	if a == b {
		return MatchAward
	}

	if a == ALIGNER || b == ALIGNER {
		return GapPenalty
	}
...
func finalize(align1 []Token, align2 []Token) ([]Token, []Token) {
	...
	for i := 0; i < len(align1); i++ {
		...
		} else if align1[i] != align2[i] && align1[i] != ALIGNER && align2[i] != ALIGNER {
			score += matchScore(align1[i], align2[i])
			symbol += " "
		} else if align1[i] == ALIGNER || align2[i] == ALIGNER {
			symbol += " "
			score += GapPenalty
		}
	}
...
func smithWaterman(seq1 []Token, seq2 []Token) ([]Token, []Token) {
	...
	for pointer[i][j] != 0 {
		if pointer[i][j] == 3 {
			...
			align1 = append(align1, ALIGNER)
			align2 = append(align2, seq2[j-1])
			j--
		} else if pointer[i][j] == 1 {
			align1 = append(align1, seq1[i-1])
			align2 = append(align2, ALIGNER)
			i--
		}
	}
